...
function offlinequiz_add_instance($offlinequiz) {
    ...
    offlinequiz_process_options($offlinequiz);
    ...
    if (is_array($offlinequiz->pdfintro) && array_key_exists('text', $offlinequiz->pdfintro)) {
        if ($draftitemid = $offlinequiz->pdfintro['itemid']) {
            ...
            $offlinequiz->pdfintro = file_save_draft_area_files(
                $draftitemid,
                $context->id,
                'mod_offlinequiz',
                'pdfintro',
...
function offlinequiz_update_instance($offlinequiz) {
    ...
    // Process the HTML editor data in pdfintro.
    if (
        property_exists($offlinequiz, 'pdfintro') && is_array($offlinequiz->pdfintro)
    ...
    $offlinequiz = $DB->get_record('offlinequiz', ['id' => $offlinequiz->id]);
...
function offlinequiz_delete_instance($id) {
    ...
    if ($results = $DB->get_records("offlinequiz_results", ['offlinequizid' => $offlinequiz->id])) {
        foreach ($results as $result) {
            offlinequiz_delete_result($result->id, $context);
        }
    }
    if ($scannedpages = $DB->get_records('offlinequiz_scanned_pages', ['offlinequizid' => $offlinequiz->id])) {
        foreach ($scannedpages as $page) {
            offlinequiz_delete_scanned_page($page, $context);
        }
    }
    if ($scannedppages = $DB->get_records('offlinequiz_scanned_p_pages', ['offlinequizid' => $offlinequiz->id])) {
    ...
    $DB->delete_records('question_references', ['usingcontextid' => $ctxid, 'component' => 'mod_offlinequiz',
        'questionarea' => 'slot']);
    if ($events = $DB->get_records('event', ['modulename' => 'offlinequiz', 'instance' => $offlinequiz->id])) {
    ...
    if ($plists = $DB->get_records('offlinequiz_p_lists', ['offlinequizid' => $offlinequiz->id])) {
        foreach ($plists as $plist) {
            $DB->delete_records('offlinequiz_participants', ['listid' => $plist->id]);
            $DB->delete_records('offlinequiz_p_lists', ['id' => $plist->id]);
        }
    }
    ...
    foreach ($tablestopurge as $table => $keyfield) {
        if (! $DB->delete_records($table, [$keyfield => $offlinequiz->id])) {
            $result = false;
        }
    }
 ...
 * @return object int
 */
function offlinequiz_grade_item_delete($offlinequiz): int {
    global $CFG;
 ...
 * @return mixed false if file not found, does not return if found - justsend the file
 */
function offlinequiz_question_pluginfile(
    $course,
    $context,
    $component,
    $filearea,
    ...
    if (!has_capability('mod/offlinequiz:viewreports', $context)) {
        ...
        if (!$result = $DB->get_record('offlinequiz_results', ['usageid' => $qubaid, 'status' => 'complete'])) {
            send_file_not_found();
        }
 ...
 * @param array $options additional options affecting the file serving.
 */
function offlinequiz_question_preview_pluginfile(
    $previewcontext,
    $questionid,
    $filecontext,
    $filecomponent,
...
function offlinequiz_pluginfile($course, $cm, $context, $filearea, $args, $forcedownload) {
    ...
    $fileareas = ['pdfs', 'participants', 'imagefiles', 'queuefiles', 'queuedatafiles'];
    ...
    if (has_capability('mod/offlinequiz:viewreports', $context)) {
        if ($filearea == 'pdfs' || $filearea == 'participants') {
            ...
            send_stored_file($file, 86400, 0, $forcedownload, ['filename' => $filename]);
        } else {
        ...
        if (
            !$scannedpages = $DB->get_records(
                'offlinequiz_scanned_pages',
                ['offlinequizid' => $offlinequiz->id, 'warningfilename' => $file->get_filename()]
            )
        ) {
            if (
                !$scannedpages = $DB->get_records('offlinequiz_scanned_pages', ['offlinequizid' => $offlinequiz->id,
        ...
        foreach ($scannedpages as $scannedpage) {
            $sql = "SELECT *
                     ...
                     WHERE id = :resultid
                       AND status = 'complete'";
            if (!$result = $DB->get_record_sql($sql, ['resultid' => $scannedpage->resultid])) {
                return false;
            }
...
function offlinequiz_num_attempt_summary($offlinequiz, $cm, $returnzero = false, $currentgroup = 0) {
    ...
    $numattempts = $DB->count_records_sql($sql, ['offlinequizid' => $offlinequiz->id]);
 ...
 */
function offlinequiz_attempt_summary_link_to_reports(
    $offlinequiz,
    $cm,
    $context,
    $returnzero = false,
 ...
 * @return mixed True if offlinequiz supports feature
 */
...
function offlinequiz_user_outline($course, $user, $mod, $offlinequiz) {
    ...
    if ($grade = $DB->get_record('offlinequiz_results', ['userid' => $user->id, 'offlinequizid' => $offlinequiz->id])) {
    ...
    foreach ($jobids as $jobid) {
        ...
        if ($files = $DB->get_records('offlinequiz_queue_data', ['queueid' => $jobid])) {
            foreach ($files as $file) {
                ...
                $DB->delete_records('offlinequiz_queue_data', ['id' => $file->id]);
            }
...
function offlinequiz_scale_used($offlinequizid, $scaleid) {
    ...
    $rec = $DB->get_record('offlinequiz', ['id' => $offlinequizid, 'grade' => -$scaleid]);
...
function offlinequiz_scale_used_anywhere($scaleid) {
    ...
    if ($scaleid && $DB->record_exists('offlinequiz', ['grade' => -$scaleid])) {
        return true;
    } else {
        return false;
    }
...
function offlinequiz_after_add_or_update($offlinequiz) {
    ...
    if (property_exists($offlinequiz, 'numgroups')) {
        for ($i = 1; $i <= $offlinequiz->numgroups; $i++) {
            if (!$group = $DB->get_record('offlinequiz_groups', ['offlinequizid' => $offlinequiz->id, 'groupnumber' => $i])) {
                $group = new stdClass();
...
function offlinequiz_print_overview($courses, &$htmlarray) {
    ...
    foreach ($offlinequizzes as $offlinequiz) {
        if ($offlinequiz->timeclose >= $now && $offlinequiz->timeopen < $now) {
            ...
            $str .= '<div class="info">' . get_string(
                'offlinequizcloseson',
                'offlinequiz',
                userdate($offlinequiz->timeclose)
            ) . '</div>';
...
function offlinequiz_grade_item_update($offlinequiz, $grades = null) {
    ...
    if (isset($offlinequiz->cmidnumber)) {
        ...
        $params = ['itemname' => $offlinequiz->name, 'idnumber' => $offlinequiz->cmidnumber];
    } else {
        $params = ['itemname' => $offlinequiz->name];
    }

    $offlinequiz->grade = $DB->get_field('offlinequiz', 'grade', ['id' => $offlinequiz->id]);
    ...
    if (
        $openreviewoptions->marks < question_display_options::MARK_AND_MAX &&
...
function offlinequiz_get_user_results($offlinequizid, $userid) {
    ...
    return $DB->get_records_select(
        'offlinequiz_results',
        "offlinequizid = :offlinequizid AND userid = :userid AND status = 'complete'",
        $params,
        'id ASC'
    );
}
...
function offlinequiz_extend_settings_navigation($settings, $offlinequiznode) {
    ...
    if ($managecapability || $viewreportscapability) {
        $node = navigation_node::create(
            get_string('tabofflinequizcontent', 'offlinequiz'),
            $url,
            navigation_node::TYPE_SETTING,
            null,
            'mod_offlinequiz_edit',
            new pix_icon('t/edit', '')
        );
    ...
    if ($viewreportscapability) {
        ...
        $node = navigation_node::create(
            get_string('tabresults', 'offlinequiz'),
            $url,
            navigation_node::TYPE_SETTING,
            null,
            'mod_offlinequiz_results',
    ...
    if ($viewreportscapability) {
        ...
        if ($active == 'mod_offlinequiz_statistics') {
            ...
            $url = new moodle_url('/mod/offlinequiz/report.php', ['id' => $settings->get_page()->cm->id, 'mode' => 'statistics']);
        }
        $node = navigation_node::create(
            get_string('tabstatistics', 'offlinequiz'),
            $url,
            navigation_node::TYPE_SETTING,
            null,
            'mod_offlinequiz_statistics',
            new pix_icon('i/report', '')
        );
    ...
    if ($viewreportscapability || $managecapability) {
        ...
        if ($participantsusage) {
            ...
            $node = navigation_node::create(
                get_string('tabattendances', 'offlinequiz'),
                $url,
                navigation_node::TYPE_SETTING,
                null,
                'mod_offlinequiz_participants',
    ...
    question_extend_settings_navigation($offlinequiznode, $settings->get_page()->cm->context)->trim_if_empty();
}

/**
 * get active tab of this page
 */
...
function offlinequiz_questions_in_use($questionids) {
    global $DB, $CFG;
    require_once($CFG->libdir . '/questionlib.php');
    [$test, $params] = $DB->get_in_or_equal($questionids);
    ...
    return $DB->record_exists_select('offlinequiz_group_questions', 'questionid ' . $test, $params) ||
           question_engine::questions_in_use($questionids, new qubaid_join(
               '{offlinequiz_results} quiza',
               'quiza.usageid',
               ''
           )) ||
           question_engine::questions_in_use($questionids, new qubaid_join(
               '{offlinequiz_groups} groupa',
...
/**
 * Add a get_coursemodule_info function in case any offlinequiz type wants to add 'extra' information
 ...
 * @return cached_cm_info|null An object on information that the courses
 *                        will know about (most noticeably, an icon).
 */
function offlinequiz_get_coursemodule_info($coursemodule) {
    global $DB;
    $dbparams = ['id' => $coursemodule->instance];
    $fields = 'id, name, intro, introformat, completionpass';
    if (!$offlinequiz = $DB->get_record('offlinequiz', $dbparams, $fields)) {
        return null;
    }
    $result = new cached_cm_info();
    $result->name = $offlinequiz->name;

    if ($coursemodule->showdescription) {
        // Convert intro to html. Do not filter cached version, filters run at display time.
        $result->content = format_module_intro('offlinequiz', $offlinequiz, $coursemodule->id, false);
    }

    // Populate the custom completion rules as key => value pairs, but only if the completion mode is 'automatic'.
    if ($coursemodule->completion == COMPLETION_TRACKING_AUTOMATIC) {
        if ($offlinequiz->completionpass) {
            $result->customdata['customcompletionrules']['completionpass'] = [
                'completionpass' => $offlinequiz->completionpass];
        } else {
            $result->customdata['customcompletionrules']['completionpass'] = [];
        }
    }
...
function mod_offlinequiz_output_fragment_add_random_question_form($args) {
    ...
    // Custom View.
    $questionbank = new mod_offlinequiz\question\bank\random_question_view(
        $contexts,
        $thispageurl,
        $course,
        $cm,
