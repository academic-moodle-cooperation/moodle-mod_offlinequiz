...
// $feedback = offlinequiz_feedback_for_grade($grade, $result->offlinequiz);
...
if ($result->userid <> $USER->id) {
    ...
    $picture = $OUTPUT->user_picture($student); //  $course->id, $student->picture, false, true);
    $table->data[] = array($picture, '<a href="'.$CFG->wwwroot.'/user/view.php?id=' . $student->id . '&amp;course=' . $course->id . '">' .
            fullname($student, true) . ' ('.$student->username.')</a>');
}
...
if ($options->marks != question_display_options::HIDDEN) {
    if ($offlinequiz->grade and $group->sumgrades) {
        $a = new stdClass;
        $percentage = round(($result->sumgrades / $group->sumgrades) * 100, 2);
        $a->grade = $grade;
        $a->maxgrade = $offlinequiz->grade;
        $rawscore = round($result->sumgrades, $offlinequizconfig->decimalpoints);
        $table->data[] = array($strscore . ':', $rawscore . '/' . round($group->sumgrades, $offlinequizconfig->decimalpoints) . ' (' . $percentage . '%)');
...
if ($isteacher or ($options->sheetfeedback == question_display_options::VISIBLE) or
    ...
    $popupoptions = array();
    $popupoptions['height'] = 1200; // optional
    $popupoptions['width'] = 1170; // optional
    ...
    if (!$isteacher) {
        ...
        foreach ($scannedpages as $scannedpage) {
            if ($scannedpage->warningfilename) {
                ...
                $fs = get_file_storage();
                $imagefile = $fs->get_file($context->id, 'mod_offlinequiz', 'imagefiles', 0, '/', $scannedpage->warningfilename);
                echo '<br/>&nbsp;<br/><img name="formimage" src="' . $CFG->wwwroot .
                "/pluginfile.php/$context->id/mod_offlinequiz/imagefiles/0/" . $imagefile->get_filename() .'" border="1" width="760" />';
            }
        }
        if ($found) {
            echo "</div><br/>";
    ...
    foreach ($scannedpages as $scannedpage) {
        if ($scannedpage->status == 'ok' || $scannedpage->status == 'submitted') {
            ...
            if ($isteacher) {
                ...
                echo $OUTPUT->action_link($url,  get_string('editscannedform', 'offlinequiz') . ' (' . get_string('page').' '.$i++ . ')',
                ...
                echo $OUTPUT->action_link($url,  get_string('linktoscannedform', 'offlinequiz') . ' (' . get_string('page').' '.$i++ . ')',
...
//  //print_paging_bar($numpages, $page, 1, 'review.php?resultid='.$result->id.'&amp;');
//  $pagingbar = new paging_bar($data->totalcount, $page, $perpage, $PAGE->url, 'page');
//  echo $OUTPUT->render($pagingbar);
//  echo '<div class="controls"><a href="review.php?resultid='.$result->id.'&amp;showall=true">';
//  print_string('showall', 'offlinequiz');
//  echo '</a></div>';
// }
