		...
		if ( $tag == "p" ) {
			...
			foreach ($rows as $row) {
				...
				foreach (array("td", "th") as $item) {
					...
					foreach ($cols as $col) {
						$c++ ;
						if ($c > 1) { $col->nodeValue = "-amp- " . $col->nodeValue; } // add & between colums
					}
					$cmax = max($cmax, $c);
				}
				$r++ ;
				if ($r > 1) { $row->nodeValue = "\\\\\n" . $row->nodeValue; } // add \\ between colums
			}
...
function offlinequiz_convert_html_to_latex($text) {
	...
	offlinequiz_convert_html_to_latex_tagreplace($dom, 'table', '');
	offlinequiz_convert_html_to_latex_tagreplace($dom, 'p', '', "\n\n");
	offlinequiz_convert_html_to_latex_tagreplace($dom, 'span', '', '');
