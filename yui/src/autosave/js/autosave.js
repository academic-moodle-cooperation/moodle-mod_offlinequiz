...
/**
 * Auto-save functionality for during offlinequiz attempts.
 *
 * @module moodle-mod_offlinequiz-autosave
 */

/**
 * Auto-save functionality for during offlinequiz attempts.
 *
 * @class M.mod_offlinequiz.autosave
 */

M.mod_offlinequiz = M.mod_offlinequiz || {};
M.mod_offlinequiz.autosave = {
    /**
     ...
     * @type String
     * @default M.cfg.wwwroot + '/mod/offlinequiz/autosave.ajax.php'
     * @private
     */
    AUTOSAVE_HANDLER: M.cfg.wwwroot + '/mod/offlinequiz/autosave.ajax.php',
    ...
    init: function(delay) {
        ...
        if (!this.form) {
            Y.log('No response form found. Why did you try to set up autosave?', 'debug', 'moodle-mod_offlinequiz-autosave');
            return;
        }
    ...
    init_tinymce: function(repeatcount) {
        if (typeof tinyMCE === 'undefined') {
            if (repeatcount > 0) {
                ...
                Y.log('Gave up looking for TinyMCE.', 'debug', 'moodle-mod_offlinequiz-autosave');
            }
            return;
        }

        Y.log('Found TinyMCE.', 'debug', 'moodle-mod_offlinequiz-autosave');
    ...
    init_tinymce_editor: function(e, editor) {
        Y.log('Found TinyMCE editor ' + editor.id + '.', 'debug', 'moodle-mod_offlinequiz-autosave');
    ...
    value_changed: function(e) {
        ...
        Y.log('Detected a value change in element ' + name + '.', 'debug', 'moodle-mod_offlinequiz-autosave');
        this.start_save_timer_if_necessary();
    },

    editor_changed: function(editor) {
        Y.log('Detected a value change in editor ' + editor.id + '.', 'debug', 'moodle-mod_offlinequiz-autosave');
        this.start_save_timer_if_necessary();
    },
    ...
    save_changes: function() {
        ...
        if (this.is_time_nearly_over()) {
            Y.log('No more saving, time is nearly over.', 'debug', 'moodle-mod_offlinequiz-autosave');
            this.stop_autosaving();
            return;
        }

        Y.log('Doing a save.', 'debug', 'moodle-mod_offlinequiz-autosave');
        if (typeof tinyMCE !== 'undefined') {
            tinyMCE.triggerSave();
        }
    ...
    save_done: function(transactionid, response) {
        ...
        Y.log('Save completed.', 'debug', 'moodle-mod_offlinequiz-autosave');
        this.save_transaction = null;

        if (this.dirty) {
            Y.log('Dirty after save.', 'debug', 'moodle-mod_offlinequiz-autosave');
            this.start_save_timer();
        }
    ...
    save_failed: function() {
        Y.log('Save failed.', 'debug', 'moodle-mod_offlinequiz-autosave');
        this.save_transaction = null;
    ...
    is_time_nearly_over: function() {
        return M.mod_offlinequiz.timer && M.mod_offlinequiz.timer.endtime &&
                (new Date().getTime() + 2 * this.delay) > M.mod_offlinequiz.timer.endtime;
    },
