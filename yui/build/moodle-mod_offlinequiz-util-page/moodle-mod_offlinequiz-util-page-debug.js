YUI.add('moodle-mod_offlinequiz-util-page', function (Y, NAME) {
 ...
 * @submodule moodle-mod_offlinequiz-util-page
 */

Y.namespace('Moodle.mod_offlinequiz.util.page');

/**
 * A collection of utility classes for use with pages.
 *
 * @class Moodle.mod_offlinequiz.util.page
 * @static
 */
Y.Moodle.mod_offlinequiz.util.page = {
    ...
    add: function(beforenode) {
        ...
        var pagehtml = M.mod_offlinequiz.resource_toolbox.get('config').pagehtml;
        ...
        YUI().use('dd-drop', function(Y) {
            var drop = new Y.DD.Drop({
                node: page,
                groups: M.mod_offlinequiz.dragres.groups
            });
            page.drop = drop;
        });
    ...
    remove: function(page, keeppagebreak) {
        ...
        var previousslot = page.previous(Y.Moodle.mod_offlinequiz.util.slot.SELECTORS.SLOT);
        if (!keeppagebreak && previousslot) {
            Y.Moodle.mod_offlinequiz.util.slot.removePageBreak(previousslot);
        }
        page.remove();
    },
