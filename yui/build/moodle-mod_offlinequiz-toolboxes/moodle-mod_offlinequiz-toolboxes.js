...
Y.extend(SECTIONTOOLBOX, TOOLBOX, {
    ...
    toggle_highlight : function(e) {
        ...
        // Get the section we're working on.
        var section = e.target.ancestor(M.mod_offlinequiz.edit.get_section_selector(Y));
        ...
        Y.one(SELECTOR.PAGECONTENT)
            .all(M.mod_offlinequiz.edit.get_section_selector(Y) + '.current ' + SELECTOR.HIGHLIGHT)
            .set('title', old_string);
        Y.one(SELECTOR.PAGECONTENT)
            .all(M.mod_offlinequiz.edit.get_section_selector(Y) + '.current ' + SELECTOR.HIGHLIGHT + ' img')
            .set('alt', old_string)
            .set('src', M.util.image_url('i/marker'));

        // Remove the highlighting from all sections.
        Y.one(SELECTOR.PAGECONTENT).all(M.mod_offlinequiz.edit.get_section_selector(Y))
            .removeClass('current');

        // Then add it if required to the selected section.
        if (!togglestatus) {
            section.addClass('current');
            value = Y.Moodle.core_course.util.section.getId(section.ancestor(M.mod_offlinequiz.edit.get_section_wrapper(Y), true));
