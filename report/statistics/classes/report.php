...
use question_bank;
use question_utils;
use offlinequiz_statistics_table;
use offlinequiz_statistics_question_table;
...
class report extends default_report {
    ...
    protected $table;
    /** @var string submode of statistics plugin. */
    protected $statmode;
    /**
     * Constructor.
     */
    ...
    public function display($offlinequiz, $cm, $course) {
        ...
        $pageoptions['statmode'] = $this->statmode;

        // When showing big tables add the JavaScript for the double scrollbar.
        if ($this->statmode == 'questionstats' || $this->statmode == 'questionandanswerstats') {
            $module = [
        ...
        if ($groupnumber > 0) {
            ...
            $offlinequiz->sumgrades = $DB->get_field(
                'offlinequiz_groups',
                'sumgrades',
        ...
        if (
            $offlinequiz->groupid == 0 && count($groups) > 1 &&
        ...
        if (empty($currentgroup)) {
            ...
            $groupstudents = get_users_by_capability(
                $this->context,
                ['mod/offlinequiz:reviewmyattempts', 'mod/offlinequiz:attempt'],
                '',
                '',
                '',
                '',
        ...
        if ($this->statmode == 'statsoverview' || $this->statmode == 'questionstats') {
            $this->table = new statistics_table();
        } else {
            $this->table = new question_answer_statistics_table();
        }
        if ($everything) {
        ...
        [$offlinequizstats, $questions, $subquestions, $s] = $this->get_offlinequiz_and_questions_stats(
            $offlinequiz,
            $currentgroup,
            $nostudentsingroup,
            $useallattempts,
        ...
        if (!$this->table->is_downloading()) {
            $this->print_header_and_tabs($cm, $course, $offlinequiz, 'statistics', $this->statmode);
            ...
            echo $OUTPUT->heading(get_string($this->statmode . 'header', 'offlinequiz_statistics') . $helpaction);
            echo $OUTPUT->box_end();

            if (!$questionid) {
                $this->print_offlinequiz_group_selector($cm, $groups, $groupnumber, $pageoptions);
                echo '<br/>';
                if ($this->statmode == 'statsoverview' && ($offlinequiz->sumgrades == -1 || $differentquestions)) {
                    echo $OUTPUT->box_start();
                    $notificationmessage = get_string('remarks', 'offlinequiz_statistics') . ":<br />";
                    if ($offlinequiz->sumgrades == -1) {
                        $notificationmessage .= '- ' . get_string(
                            'differentsumgrades',
        ...
        if ($everything) { // Implies is downloading.
            ...
            if ($this->statmode == 'statsoverview') {
                $this->download_offlinequiz_info_table($offlinequizinfo);
            } else if ($this->statmode == 'questionstats') {
                if ($s) {
            ...
            } else if ($this->statmode == 'questionandanswerstats') {
                if ($s) {
                    $this->output_offlinequiz_structure_analysis_table($s, $questions, $subquestions);
    ...
    private function print_offlinequiz_group_selector($cm, $groups, $groupnumber, $pageoptions) {
        ...
        $urlparams = ['id' => $cm->id, 'mode' => 'statistics', 'statmode' => $pageoptions['statmode']];
    ...
    protected function output_individual_question_data($offlinequiz, $question) {
        ...
        $questionstatstable->align = ['left', 'right'];
    ...
    protected function output_individual_question_response_analysis(
        ...
        $qtable = new question_table($question->id);
        ...
        if (!$this->table->is_downloading()) {
            ...
            if ($this->table->is_downloading() == 'xhtml') {
                $questiontabletitle = get_string(
                    'analysisofresponsesfor',
                    'offlinequiz_statistics',
                    $questiontabletitle
                );
            }
        ...
        foreach ($responesstats->responseclasses as $partid => $partclasses) {
            ...
            foreach ($partclasses as $responseclassid => $responseclass) {
                ...
                if (empty($responsesdata)) {
                    $rowdata->part = $letterstr[$counter] . ')';
                    $rowdata->response = $responseclass->responseclass;
                    $this->output_response_data(
                        $rowdata,
                        $responseclass->fraction,
                        0,
                        $letterstr,
                        $counter,
                        $counter2,
    ...
    protected function output_offlinequiz_info_table($offlinequizinfo) {
        ...
        $offlinequizinfotable->align = ['left', 'right'];
        $offlinequizinfotable->attributes['class'] = 'generaltable titlesleft';
        $offlinequizinfotable->data = [];

        foreach ($offlinequizinfo as $heading => $value) {
             $offlinequizinfotable->data[] = [$heading, $value];
        }
     ...
     */
    protected function get_emtpy_stats(
        $questions,
     ...
     *      - array $qstats The statistics for each question.
     */
    protected function compute_stats(
        $offlinequizid,
        $currentgroup,
        $nostudentsingroup,
        $useallattempts,
        ...
        if ($s == 0) {
            return $this->get_emtpy_stats(
                $questions,
                $firstattempts->countrecs,
        ...
        $qstats = new question_stats($questions, $s, $summarksavg);
        ...
        return [$s, $offlinequizstats, $qstats];
    }
     ...
     */
    protected function try_loading_cached_stats(
        $offlinequiz,
        $currentgroup,
        $nostudentsingroup,
        $useallattempts,
        ...
        if ($offlinequiz->groupid) {
            $offlinequizstats = $DB->get_record_select(
                'offlinequiz_statistics',
                'offlinequizid = ? AND offlinegroupid = ? AND groupid = ? AND allattempts = ? AND timemodified > ?',
                [$offlinequiz->id, $offlinequiz->groupid, $currentgroup, $useallattempts, $timemodified]
            );
        } else {
            $offlinequizstats = $DB->get_record_select(
                'offlinequiz_statistics',
                'offlinequizid = ? AND offlinegroupid = 0 AND groupid = ? AND allattempts = ? AND timemodified > ?',
                [$offlinequiz->id, $currentgroup, $useallattempts, $timemodified]
            );
        }

        if (!$offlinequizstats) {
            // No cached data found.
            return [null, $questions, null, null];
        }
        ...
        $subquestions = [];
        $questionstats = $DB->get_records(
            'offlinequiz_q_statistics',
            ['offlinequizstatisticsid' => $offlinequizstats->id]
        );
     ...
     */
    protected function cache_stats(
        $offlinequizid,
        $currentgroup,
        $offlinequizstats,
        $questions,
     ...
     *     - $s Number of attempts included in the stats.
     */
    protected function get_offlinequiz_and_questions_stats(
        $offlinequiz,
        $currentgroup,
        $nostudentsingroup,
        $useallattempts,
        ...
        [$offlinequizstats, $questions, $subquestions, $s]
          = $this->try_loading_cached_stats(
              $offlinequiz,
              $currentgroup,
              $nostudentsingroup,
              $useallattempts,
        ...
        if (is_null($offlinequizstats)) {
            [$s, $offlinequizstats, $qstats] = $this->compute_stats(
                $offlinequiz->id,
                $currentgroup,
                $nostudentsingroup,
                $useallattempts,
            ...
            if ($s) {
                ...
                $offlinequizstatisticsid = $this->cache_stats(
                    $offlinequiz->id,
                    $currentgroup,
                    $offlinequizstats,
                    $questions,
                ...
                $this->analyse_responses(
                    $offlinequizstatisticsid,
                    $offlinequiz->id,
                    $currentgroup,
                    $nostudentsingroup,
                    $useallattempts,
     ...
     * @return void
     */
    protected function analyse_responses(
        $offlinequizstatisticsid,
        $offlinequizid,
        $currentgroup,
        $nostudentsingroup,
        ...
        foreach ($subquestions as $question) {
            if (
                !question_bank::get_qtype($question->qtype, false)->can_analyse_responses() ||
    ...
    protected function everything_download_options() {
        ...
        if ($this->table->baseurl) {
            return $OUTPUT->download_dataformat_selector(
                ...
                $this->table->baseurl->out_omit_querystring(),
                'download',
     ...
     */
    protected function output_caching_info(
        $offlinequizstats,
        $offlinequizid,
        $currentgroup,
        $groupstudents,
    ...
    protected function clear_cached_data($offlinequizid, $currentgroup, $useallattempts, $offlinegroupid) {
        ...
        if ($offlinegroupid) {
            $todelete = $DB->get_records_menu(
                'offlinequiz_statistics',
                ['offlinequizid' => $offlinequizid, 'offlinegroupid' => $offlinegroupid,
                'groupid' => $currentgroup,
                'allattempts' => $useallattempts],
                '',
                'id, 1'
            );
        } else {
            $todelete = $DB->get_records_menu('offlinequiz_statistics', ['offlinequizid' => $offlinequizid,
    ...
    public function get_navigation_key_from_statmode($statmode): string {
        switch ($statmode) {
            case 'statsoverview':
                return 'tabstatsoverview';
            case 'questionstats':
                return 'tabquestionstats';
            case 'questionandanswerstats':
                return 'tabquestionandanswerstats';
            default:
                throw new \coding_exception('Invalid statmode');
        }
    }
}
 ...
 */
function offlinequiz_statistics_attempts_sql(
    $offlinequizid,
    $currentgroup,
    $groupstudents,
    $allattempts = true,
    ...
    $qaparams = ['offlinequizid' => $offlinequizid, 'offlinequizstatefinished' => 'complete'];
    ...
    if (!empty($currentgroup) && $groupstudents) {
        [$grpsql, $grpparams] = $DB->get_in_or_equal(
            array_keys($groupstudents),
            SQL_PARAMS_NAMED,
            'u'
        );
 ...
 * @return qubaid_join
 */
function offlinequiz_statistics_qubaids_condition(
    $offlinequizid,
    $currentgroup,
    $groupstudents,
    $allattempts = true,
