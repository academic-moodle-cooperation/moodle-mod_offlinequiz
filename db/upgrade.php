...
/*    
    if ($oldversion < 2015060500)
     {
        ...
        // Launch rename table for offlinequiz_q_instances.
        $dbman->rename_table($table, 'offlinequiz_slots');
    ...
    if ($oldversion < 2015060501) {
        ...
        // Define field slot to be added to offlinequiz_slots.
        $table = new xmldb_table('offlinequiz_slots');
    ...
    if ($oldversion < 2015060502) {
        ...
        // Define field page to be added to offlinequiz_slots.
        $table = new xmldb_table('offlinequiz_slots');
        $field = new xmldb_field('page', XMLDB_TYPE_INTEGER, '10', null, null, null, null, 'offlinequizid');
    ...
    if ($oldversion < 2015060503) {
        ...
        // Use the information in the old offlinequiz.questions column to fill in the
        ...
        if ($numofflinequizzes > 0) {
            ...
            $numberdone = 0;
            $offlinequizzes = $DB->get_recordset('offlinequiz', null, 'id', 'id,questions,sumgrades');
            foreach ($offlinequizzes as $offlinequiz) {
                if ($offlinequiz->questions === '') {
                    $questionsinorder = array();
                } else {
                    $questionsinorder = explode(',', $offlinequiz->questions);
                }
    
                $questionidtoslotrowid = $DB->get_records_menu('offlinequiz_slots',
                        array('offlinequizid' => $offlinequiz->id), '', 'id, questionid');
                ...
                foreach ($questionsinorder as $questionid) {
                    ...
                    if ($questionid === '') {
                        ...
                        // the offlinequiz system used to do. See MDL-45321.
                        continue;
                    }
    
                    $key = array_search($questionid, $questionidtoslotrowid);
                    if ($key !== false) {
                        // Normal case. offlinequiz_slots entry is present.
                        // Just need to add slot and page.
                        $offlinequizslot = new stdClass();
                        $offlinequizslot->id   = $key;
                        $offlinequizslot->slot = $slot;
                        $offlinequizslot->page = $currentpage;
                        $DB->update_record('offlinequiz_slots', $offlinequizslot);
                        ...
                        // offlinequiz.questions, but there was not an entry for it in
                        // offlinequiz_slots (formerly offlinequiz_q_instances).
                        // Previously, if such question ids were found, then
                        // starting an attempt at the offlinequiz would throw an exception.
                        ...
                        if ($defaultmark === false) {
                            debugging('During upgrade, detected that question ' .
                                    $questionid . ' was listed as being part of offlinequiz ' .
                                    $offlinequiz->id . ' but this question no longer exists. Ignoring it.', DEBUG_NORMAL);
                        ...
                        debugging('During upgrade, detected that question ' .
                                $questionid . ' was listed as being part of offlinequiz ' .
                                        $offlinequiz->id . ' but there was not entry for it in ' .
                                        'offlinequiz_q_instances/offlinequiz_slots. Creating an entry with default mark.', DEBUG_NORMAL);
                        $offlinequizslot = new stdClass();
                        $offlinequizslot->offlinequizid     = $offlinequiz->id;
                        $offlinequizslot->slot       = $slot;
                        $offlinequizslot->page       = $currentpage;
                        $offlinequizslot->questionid = $questionid;
                        $offlinequizslot->maxmark    = $defaultmark;
                        $DB->insert_record('offlinequiz_slots', $offlinequizslot);
                ...
                // offlinequiz_slots rows linked to this offlinequiz.
                if (!empty($questionidtoslotrowid)) {
                    debugging('During upgrade, detected that questions ' .
                            implode(', ', array_values($questionidtoslotrowid)) .
                            ' had instances in offlinequiz ' . $offlinequiz->id . ' but were not actually used. ' .
                            'The instances have been removed.', DEBUG_NORMAL);
    
                    $DB->delete_records_list('offlinequiz_slots', 'id', array_keys($questionidtoslotrowid));
                    $problemfound = true;
                }
    
                // If there were problems found, we probably need to re-compute
                // offlinequiz.sumgrades.
                if ($problemfound) {
                    // C/f the offlinequiz_update_sumgrades function in locallib.php,
                    // but note that what we do here is a bit simpler.
                    $newsumgrades = $DB->get_field_sql(
                            "SELECT SUM(maxmark)
                               FROM {offlinequiz_slots}
                              WHERE offlinequizid = ?",
                            array($offlinequiz->id));
                    if (!$newsumgrades) {
                        $newsumgrades = 0;
                    }
                    if (abs($newsumgrades - $offlinequiz->sumgrades) > 0.000005) {
                        debugging('Because of the previously mentioned problems, ' .
                                'sumgrades for offlinequiz ' . $offlinequiz->id .
                                ' was changed from ' . $offlinequiz->sumgrades . ' to ' .
                                $newsumgrades . ' You should probably check that this offlinequiz is still working: ' .
                                $CFG->wwwroot . '/mod/offlinequiz/view.php?q=' . $offlinequiz->id . '.', DEBUG_NORMAL);
                                $DB->set_field('offlinequiz', 'sumgrades', $newsumgrades, array('id' => $offlinequiz->id));
                    }
                }
                ...
                $numberdone++;
                $pbar->update($numberdone, $numofflinequizzes,
                        "Upgrading offlinequiz structure - {$numberdone}/{$numofflinequizzes}.");
            }
        ...
        // Offlinequiz savepoint reached.
        upgrade_mod_savepoint(true, 2015060503, 'offlinequiz');
    }
    
    if ($oldversion < 2015060504) {
    
        // If, for any reason, there were any offlinequiz_slots missed, then try
        ...
        // any such offlinequiz_slots rows must refer to a non-existent offlinequiz id, so
        // delete them.
        $DB->delete_records_select('offlinequiz_slots',
                'NOT EXISTS (SELECT 1 FROM {offlinequiz} q WHERE q.id = offlinequizid)');
    
        // Offlinequiz savepoint reached.
        upgrade_mod_savepoint(true, 2015060504, 'offlinequiz');
    
        // Now, if any offlinequiz_slots rows are left with slot or page NULL, something
        // is badly wrong.
        if ($DB->record_exists_select('offlinequiz_slots', 'slot IS NULL OR page IS NULL')) {
            throw new coding_exception('Something went wrong in the offlinequiz upgrade step for MDL-43749. ' .
                    'Some offlinequiz_slots still contain NULLs which will break the NOT NULL constraints we need to add. ' .
                    'Please report this problem at http://tracker.moodle.org/ so that it can be investigated. Thank you.');
        }
    }
    
    if ($oldversion < 2015060505) {
    
        // Changing nullability of field slot on table offlinequiz_slots to not null.
        $table = new xmldb_table('offlinequiz_slots');
        ...
        // Offlinequiz savepoint reached.
        upgrade_mod_savepoint(true, 2015060505, 'offlinequiz');
    }
    
    if ($oldversion < 2015060506) {
    
        // Changing nullability of field page on table offlinequiz_slots to not null.
        $table = new xmldb_table('offlinequiz_slots');
        $field = new xmldb_field('page', XMLDB_TYPE_INTEGER, '10', null, XMLDB_NOTNULL, null, null, 'offlinequizid');
    
        // Launch change of nullability for field page.
        $dbman->change_field_notnull($table, $field);
    
        // Offlinequiz savepoint reached.
        upgrade_mod_savepoint(true, 2015060506, 'offlinequiz');
    }
    
    if ($oldversion < 2015060507) {
    
        // Define index offlinequizid-slot (unique) to be added to offlinequiz_slots.
        $table = new xmldb_table('offlinequiz_slots');
        $index = new xmldb_index('offlinequizid-slot', XMLDB_INDEX_UNIQUE, array('offlinequizid', 'slot'));
    
        // Conditionally launch add index offlinequizid-slot.
        if (!$dbman->index_exists($table, $index)) {
            $dbman->add_index($table, $index);
        }
    
        // Offlinequiz savepoint reached.
        upgrade_mod_savepoint(true, 2015060507, 'offlinequiz');
    }
