...
class offlinequiz_ilog_upgrader {
    ...
    public function convert_all_offlinequiz_attempts() {
        ...
        $offlinequizzes = $DB->get_records('offlinequiz', ['needsilogupgrade' => 1]);
    ...
    public function update_all_group_template_usages($offlinequiz) {
        ...
        foreach ($groups as $group) {
            if (
                $attempt = $DB->get_record('offlinequiz_attempts', ['offlinequiz' => $offlinequiz->id,
    ...
    public function update_all_results_and_logs($offlinequiz) {
        ...
        $attempts = $DB->get_records('offlinequiz_attempts', ['offlinequiz' => $offlinequiz->id,
        ...
        [$maxquestions, $maxanswers, $formtype, $questionsperpage] = offlinequiz_get_question_numbers($offlinequiz, $groups);
        $transaction = $DB->start_delegated_transaction();

        foreach ($attempts as $attempt) {
            $group = $DB->get_record('offlinequiz_groups', ['offlinequizid' => $offlinequiz->id,
                                                                 'number' => $attempt->groupid]);
            $attemptlog = $DB->get_record('offlinequiz_i_log', ['offlinequiz' => $offlinequiz->id,
            ...
            if (
                !$oldresult = $DB->get_record('offlinequiz_results', ['offlinequizid' => $result->offlinequizid,
            ...
            $DB->set_field('offlinequiz_attempts', 'resultid', $result->id, ['id' => $attempt->id]);
            ...
            if ($quba = question_engine::load_questions_usage_by_activity($result->usageid)) {
                ...
                if ($group->numberofpages == 1) {
                    ...
                    AND page > 0";
                    $params = ['offlinequizid' => $offlinequiz->id, 'attemptid' => $attempt->id];
        ...
        $DB->set_field('offlinequiz', 'needsilogupgrade', 0, ['id' => $offlinequiz->id]);
        $transaction->allow_commit();

        // We start a new transaction for the remaining i_log entries.
        $otherlogs = $DB->get_records('offlinequiz_i_log', ['offlinequiz' => $offlinequiz->id, 'attempt' => 0]);
        $transaction = $DB->start_delegated_transaction();

        foreach ($otherlogs as $pagelog) {
            [$status, $error] = $this->get_status_and_error($pagelog->error);
            $rawdata = $pagelog->rawdata;
...
class offlinequiz_attempt_upgrader extends question_engine_attempt_upgrader {
    ...
    protected function get_quiz_ids() {
        ...
        if (is_readable($partialupgradefile)) {
            ...
            if (function_exists($partialupgradefunction)) {
                ...
                [$test, $params] = $DB->get_in_or_equal($quizids);
    ...
    public function convert_all_quiz_attempts() {
        ...
        foreach ($quizids as $quizid) {
            ...
            $quiz = $DB->get_record('offlinequiz', ['id' => $quizid], '*', MUST_EXIST);
     ...
     */
    protected function convert_quiz_attempt(
        $quiz,
        $attempt,
    ...
    public function save_usage($preferredbehaviour, $attempt, $qas, $quizlayout) {
        ...
        foreach (explode(',', $quizlayout) as $questionid) {
            ...
            if (textlib::strlen($qa->questionsummary) > question_bank::MAX_SUMMARY_LENGTH) {
                ...
                $qa->questionsummary = textlib::substr(
                    $qa->questionsummary,
                    0,
                    question_bank::MAX_SUMMARY_LENGTH - 3
                ) . '...';
            }
    ...
    protected function get_converter_class_name($question, $quiz, $qsessionid) {
        ...
        if ($question->qtype == 'deleted') {
            ...
            $params = ['questionid' => $question->id, 'randomid' => "random{$question->id}-%"];
    ...
    public function supply_missing_question_attempt($quiz, $attempt, $question) {
        ...
        $qbehaviourupdater = new $converterclass(
            $quiz,
            $attempt,
            $question,
            null,
    ...
    protected function decode_random_attempt($qstates, $maxmark) {
        ...
        foreach ($qstates as $i => $state) {
            ...
            [$randombit, $realanswer] = explode('-', $state->answer, 2);
...
function offlinequiz_remove_redundant_q_instances() {
    ...
    $offlinequizzes = $DB->get_records('offlinequiz', [], 'id', 'id');
    foreach ($offlinequizzes as $offlinequiz) {
        $transaction = $DB->start_delegated_transaction();

        $qinstances = $DB->get_records('offlinequiz_q_instances', ['offlinequiz' => $offlinequiz->id], 'groupid');
        // First delete them all.
        $DB->delete_records('offlinequiz_q_instances', ['offlinequiz' => $offlinequiz->id]);

        // Now insert one per question.
        foreach ($qinstances as $qinstance) {
            if (
                !$DB->get_record('offlinequiz_q_instances', ['offlinequiz' => $qinstance->offlinequiz,
